...
class TestGitSource:
    def test_pull(self, fake_run):
        git = GitSource("git://my-source", "source_dir")
        git.pull()
    ...
    def test_add_error(self, fake_check_output):
        ...
        with pytest.raises(errors.GitCommandError) as raised:
            git.add("file")
        assert raised.value.command == ["git", "-C", "source_dir", "add", "file"]
        assert raised.value.exit_code == 44
    ...
    def test_init_error(self, fake_check_output):
        ...
        assert str(raised.value) == (
            "Git command ['git', '-C', 'source_dir', 'init'] failed with code 44: "
            "git: some error"
        )
    ...
    def test_pull_with_depth(self, fake_run):
        git = GitSource("git://my-source", "source_dir", source_depth=2)
    ...
    def test_pull_branch(self, fake_run):
        git = GitSource("git://my-source", "source_dir", source_branch="my-branch")
        git.pull()
    ...
    def test_pull_tag(self, fake_run):
        git = GitSource("git://my-source", "source_dir", source_tag="tag")
        git.pull()
...
class TestGitConflicts(GitBaseTestCase):
    ...
    def test_git_conflicts(self):
        ...
        git = GitSource(repo, working_tree, silent=True)
